...
if (isset($_POST['form1'])) {
	...
	if (empty($_POST['nhanhieu'])) {
		$valid = 0;
		$error_message .= "Tên Nhà cung cấp không được để trống<br>";
	} else {
		if (empty($_POST['nhanhieu'])) {
			$valid = 0;
			$error_message .= "Tên Nhà cung cấp không được để trống<br>";
			...
			$apiUrl = 'http://localhost:8080/api-admin/controller-nhacungcap/edit';
			$ch = curl_init($apiUrl);
			...
			if ($responseData === null) {
				...
				$success_message = 'Thay đổi Tên Nhà cung cấp thành công.';
			}
		}
	}
}
...
$apiUrl = 'http://localhost:8080/api-admin/controller-nhacungcap/show-id';
$data = array(
	'id' => $_REQUEST['id']
);
$ch = curl_init($apiUrl);
...
// print_r( $responseData);
	$nhanhieu = $responseData['data']['tenNcc'];
